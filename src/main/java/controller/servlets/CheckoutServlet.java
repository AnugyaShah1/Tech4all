	        ...
	        try {
	         ...
	         // Check stock availability
	            for (Cart cartItem : cartItems) {
	                ProductModel product = databaseController.getProductDetail(cartItem.getProductId());
	                if (product.getStock() < cartItem.getQuantity()) {
	                    response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Insufficient stock for product: " + product.getProd_name());
	                    return;
	                }
	            }
